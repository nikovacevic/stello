...
This app is both a stello application (the dogfood ``terminal`` app in the repo's
``stello.yaml``) and a built-in command: it ships in the package so ``stello terminal``
launches it directly, with no project initialized. It reuses stello's own library to:
...
class StelloTUI(App):
    ...
    def _would_relaunch_tui(self, app: Application) -> bool:
        ...
        Detects the terminal app whether it's declared as the in-repo dogfood app or a
        thin shim, by looking for this module's import marker — so the guard holds for both
        ``stello run terminal`` and the built-in ``stello terminal``.
        """
        if self.current_project_path is None:
            return False
        try:
            script = app.resolved_script(self.current_project_path)
        except Exception:
            return False
        if script == SELF_SCRIPT:
            return True
        try:
            return "stello._apps.terminal" in script.read_text(encoding="utf-8", errors="ignore")
        except OSError:
            return False
...
def run(theme: str = "dark", compact: bool = False) -> None:
    """Launch the Textual TUI. Called by ``stello terminal`` and the dogfood shim."""
