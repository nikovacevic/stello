...
Like the TUI, this is both the dogfood ``stello`` app in the repo's ``stello.yaml`` and a
built-in command: it ships in the package, so ``stello dashboard`` launches it directly
with no project initialized (also ``stello run stello`` via the dogfood project).
...
def _would_relaunch_dashboard(project: str, app: Application) -> bool:
    ...
    Detects the dashboard app whether declared as the in-repo dogfood app or a thin shim,
    via this module's import marker — so the guard holds for both ``stello run stello`` and
    the built-in ``stello dashboard``.
    """
    try:
        script = app.resolved_script(core.project_path(project))
    except Exception:  # noqa: BLE001 - if we can't tell, fall through and let it try
        return False
    if script == SELF_SCRIPT:
        return True
    try:
        return "stello._apps.dashboard" in script.read_text(encoding="utf-8", errors="ignore")
    except OSError:
        return False
...
def _index() -> None:
    ...
    in-process from anywhere — ``stello dashboard`` or the dogfood shim. The auto-index path
    rebuilds the page by re-running ``sys.argv[0]`` as ``__main__``, which only works when the
    entry point *is* this script; a page route builds it by calling this function instead.
    """
    build()


def run(port: int = 8080, theme: str = "dark") -> None:
    """Launch the NiceGUI dashboard. Called by ``stello dashboard`` and the dogfood shim."""
...
def main() -> None:
    """argparse entry point for running this module as a script (the dogfood app)."""
