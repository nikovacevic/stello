...
lists as a dependency): it browses initialized projects (left pane), lists the applications
in whichever project you're browsing (middle pane), and opens an application into a run pane
...
- Enter (or a click) on an application opens it into the run pane;
...
Projects are just namespaces here: browsing them (arrow keys or a click) is purely
non-destructive.
...
def _browse(name: str | None) -> None:
    """Preview a project's applications (runtime handler)."""
    _load_project(name)
    apps_panel.refresh()
    _refresh_bar()
    ...
    with ui.element("div").classes("pane " + ("pane-active" if focused else "")):
        ...
        with ui.element("div").classes("pane-body"):
            for i, info in enumerate(infos):
                ...
                row.on("click", lambda n=info.name, idx=i: (_focus_select("projects", idx), _browse(n)))
                with row:
                    ui.label(info.name)
...
def build() -> None:
    ...
    with ui.column().classes("w-full h-screen p-0 gap-0"):
        ...
        with ui.element("div").classes("footer w-full"):
            ui.label("↑↓ navigate   ·   Tab / Shift+Tab switch pane   ·   Enter open app   ·   click to select")
...
def _init() -> None:
    """Load state for the first project before the panes render."""
    infos = core.list_projects()
    if not infos:
        _load_project(None)
        return
...
def _index() -> None:
    ...
    ``sys.argv[0]`` as ``__main__``, which relies on the entry point *being* this script; a
    page route builds it by calling this function instead.
    """
    build()
